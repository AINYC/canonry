...
canonry mcp install --client cursor --read-only      # scope to the 96 read API tools
...
a parallel surface. v1 exposes 153 curated API tools (96 read, 57 write) — including
...
- `packages/canonry/src/mcp/tool-registry.ts` — single source of truth for all 153 API tools, each tagged with a `tier`
