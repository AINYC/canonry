...
describe('buildAdsOperatorTools', () => {
  it('adds the one-call context tool and narrows read-only mode to the ads operator allow-list', () => {
    ...
    expect(names).not.toContain(CanonryMcpToolNames.canonry_ads_campaign_pause)
    expect(names).not.toContain(CanonryMcpToolNames.canonry_ads_campaign_activate_tree)
    expect(names).not.toContain(CanonryMcpToolNames.canonry_ads_operation_resume_activation)
  ...
  it('keeps only selected write tools when full scope is requested', () => {
    ...
    expect(names).toContain(CanonryMcpToolNames.canonry_ads_campaign_create)
    expect(names).toContain(CanonryMcpToolNames.canonry_ads_campaign_update)
    expect(names).toContain(CanonryMcpToolNames.canonry_ads_campaign_pause)
    expect(names).toContain(CanonryMcpToolNames.canonry_ads_campaign_activate_tree)
    expect(names).toContain(CanonryMcpToolNames.canonry_ads_operation_resume_activation)
    expect(names).toContain(CanonryMcpToolNames.canonry_ads_ad_group_create)
    expect(names).toContain(CanonryMcpToolNames.canonry_ads_ad_create)
