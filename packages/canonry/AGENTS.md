...
| `src/commands/google.ts` | Google commands, including project-bound GSC social/video property list, bind, remove, sync, and aggregate/per-property performance reads. |
...
| `src/mcp/tool-registry.ts` | All 149 API tools, including isolated GSC social/video property list, performance, bind, remove, and sync tools; each is tagged with a `tier` (`core` or one of the toolkit names) |
...
| `src/gsc-platform-sync.ts` | Isolated social/video Search Console worker. The run's project-bound `sourceId` must match the bound property; it calls standard Search Analytics only and never website GSC, URL inspection, coverage, or sitemap paths. |
