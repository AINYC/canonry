...
| `src/agent-plugin.ts` | Best-effort, secret-free, user-global native Canonry plugin detection for Claude Code JSON settings and Codex TOML settings. It distinguishes configured clients from verified cache assets and records each verified manifest version so doctor/nudges can enforce runtime lockstep; `serve` passes a live provider, and detection never controls filesystem writes. |
...
| `src/mcp/tool-registry.ts` | All API tools (142, including `canonry_ga_measurement_analysis`, `canonry_analytics_sources`, `canonry_visibility_stats`, `canonry_visibility_compare`, saved research query runs, `canonry_content_brief`, `canonry_content_map`, `canonry_report`, `canonry_ads_delivery_diagnostics`, `canonry_ads_live_delivery`, `canonry_analytics_metrics`, `canonry_gbp_places`, `canonry_backlinks_sources`, the guarded ads lifecycle and activation-recovery tools, and the `canonry_technical_aeo_*` site-audit tools), each tagged with a `tier` (`core` or one of the toolkit names) |
...
| `src/commands/backlinks.ts` | Backlinks commands: `backlinks install`, `doctor`, `status`, `sync`, `list` (source-aware via `--source commoncrawl\|bing-webmaster`), `sources` (per-source availability), `bing-sync` (pull Bing inbound links), `extract`, `releases`, `cache prune` |
| `src/bing-backlinks-sync.ts` | `executeBingBacklinkSync`, the per-project Bing inbound-links sync: lists the site's pages with inbound links (`GetLinkCounts`), pulls the external linking URLs per page (`GetUrlLinks`), folds them into per-linking-domain rows (`aggregateInboundLinksByDomain`, pure + tested), and writes `source='bing-webmaster'` backlink rows + a summary stamped with a synthetic per-UTC-day window (`bing-YYYY-MM-DD`). Wired to the manual `POST /backlinks/bing-sync` route and the scheduled `backlinks-sync` kind (for Bing-connected projects) via `server.ts`. |
