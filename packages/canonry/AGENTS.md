...
| `src/commands/traffic.ts` | Server-side traffic commands: pull-source connects for Cloud Run / WordPress / Vercel plus local-only `traffic connect cloudflare` direct-push setup. Cloudflare writes secret-free `worker.js` / `wrangler.toml` artifacts and safely updates recognizable generated files. `--deploy` feature-detects Wrangler safety flags before connect, requires `--zone-id --confirm-route --confirm-fail-open`, installs the locally held bearer/HMAC through a mode-0600 temporary file, and removes it in `finally`. Wrangler deploys no route. The operator attaches the exact route manually with Request limit failure mode set to Fail open. The command must refuse auto-deploy when CLI and server do not share the credential store. Other commands: `traffic sync`, async `backfill`, explicit `reset --advance-to-now`, `sources`, `status`, and windowed `events`. |
