...
| `src/mcp/tool-registry.ts` | All API tools (143, including `canonry_ga_ai_referral_daily`, `canonry_ga_measurement_analysis`, `canonry_analytics_sources`, `canonry_visibility_stats`, `canonry_visibility_compare`, saved research query runs, `canonry_content_brief`, `canonry_content_map`, `canonry_report`, `canonry_ads_delivery_diagnostics`, `canonry_analytics_metrics`, `canonry_gbp_places`, `canonry_backlinks_sources`, the guarded ads lifecycle and activation-recovery tools, and the `canonry_technical_aeo_*` site-audit tools), each tagged with a `tier` (`core` or one of the toolkit names) |
...
| `src/commands/ga.ts` | GA4 commands: `ga sync`, `ga traffic`, `ga status`, `ga ai-referral-daily` (AI sessions per day and per source, landing pages summed inside ONE attribution dimension so the totals match `ga traffic`), `ga ai-referral-history` (raw per-landing-page detail rows, never a total), `ga social-referral-history`, `ga social-referral-summary`, `ga attribution` |
