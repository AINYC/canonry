...
/**
 * Per-project Cloudflare Worker traffic connection. The Worker is deployed
 * by the operator (or by canonry in the Phase 2 auto-deploy story) onto
 * their Cloudflare zone; it forwards filtered requests to the canonry
 * `traffic/cloudflare/ingest` endpoint using `bearerToken` for
 * Authorization and `hmacSecret` for the per-request HMAC. Both secrets
 * are generated by canonry at connect time and embedded into the
 * generated Worker script. The DB only stores the sha256 of the bearer
 * for verification; both cleartext secrets live here only.
 */
export interface CloudflareTrafficConnectionConfigEntry {
  ...
  /** Operator-supplied Cloudflare zone id. Phase 2 auto-deploy uses it; Phase 1 informational only. */
  zoneId: string | null
  /** Operator-supplied Cloudflare account id. Required for Phase 2 auto-deploy; Phase 1 informational. */
