...
| `src/engine-routes.ts` | Generic OpenAI-compatible connection and route contracts. Persisted `route:*` entries are text-only; only host-owned `implicit-native` adapters can be measurement-ready. The removed OpenRouter preset and measurement-policy fields are rejected by strict schemas rather than downgraded. |
