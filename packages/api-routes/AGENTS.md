...
| `src/ads-live-delivery.ts` | Pure live-vs-stored comparison engine (no DB/clock/network; the `gbp-summary.ts` precedent) behind `GET /ads/live-delivery` — `buildFieldDeltas` (status / reviewStatus / name; nothing when one side is absent, since `presence` already says it), `buildMetricDeltas` (per-date union of provider rows and in-window stored rollups; the provider's disjoint buckets for one date are summed — impressions/clicks/spend/conversions are additive, the ratio metrics ctr/cpc/cpm are NOT and survive only verbatim in `liveMetrics` — and spend is converted from the insights API's decimal units to stored micros so both sides are commensurable), `buildLiveEntityComparison`, and `summarizeLiveDrift`. |
