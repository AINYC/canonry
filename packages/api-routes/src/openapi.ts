...
const routeCatalog: OpenApiOperation[] = [
  ...
  {
    method: 'get',
    path: '/api/v1/projects/{name}/visibility-report',
    summary: 'Read a consistently scoped AI visibility report',
    description: 'Stored official evidence only. Summary, trend, queries and competitor metrics use one selection and server-owned denominators. All classes returns separate populations, never a pooled rate. Market scopes select frozen execution edges; groups remain property collections. Research and probe answers are excluded. Material plan changes retain the prior measured definition until a new sweep.',
    tags: ['measurement-plans'],
    parameters: [
      nameParameter,
      { name: 'mode', in: 'query', description: 'Auto selects the current project measurement mode.', schema: { type: 'string', enum: ['auto', 'simple', 'advanced'], default: 'auto' } },
      { name: 'scope', in: 'query', description: 'A project, property collection, frozen market edge set, or property.', schema: { type: 'string', enum: ['project', 'group', 'market', 'property'], default: 'project' } },
      { name: 'scopeKey', in: 'query', description: 'Stable scope key. Required for non-project scopes.', schema: stringSchema },
      { name: 'queryClass', in: 'query', description: 'All returns independent populations, never pooled coverage.', schema: { type: 'string', enum: ['non-brand', 'branded', 'unknown', 'all'], default: 'non-brand' } },
      ...Object.entries({ provider: 'Exact answer engine.', model: 'Exact observed model.', location: 'Exact location label, or none for no location.', from: 'Inclusive ISO start time.', to: 'Inclusive ISO end time.', runId: 'Exact official measured run, independent of browser drawer state.', queryKey: 'Exact query-context key for answer detail.', queryId: 'Tracked query identity for answer detail; mutually exclusive with queryKey.', cursor: 'Opaque page cursor. Reuse with the identical selection.', search: 'Query list search only. Does not change summary or trend denominators.' }).map(([name, description]) => ({ name, in: 'query' as const, description, schema: stringSchema })),
      { name: 'revision', in: 'query', description: 'Exact frozen measurement revision.', schema: { type: 'integer', minimum: 1 } },
      { name: 'limit', in: 'query', description: 'Maximum query or answer rows per page.', schema: { type: 'integer', minimum: 1, maximum: 100, default: 50 } },
    ],
    responses: {
