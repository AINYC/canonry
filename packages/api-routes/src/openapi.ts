...
const routeCatalog: OpenApiOperation[] = [
  ...
  {
    method: 'get',
    path: '/api/v1/projects/{name}/google/gsc/platform-properties',
    summary: 'List enrolled social and video Search Console properties',
    tags: ['google'], parameters: [nameParameter],
    responses: { 200: jsonResponse('Platform properties returned.', 'GscPlatformPropertyListResponseDto') },
  },
  {
    method: 'put',
    path: '/api/v1/projects/{name}/google/gsc/platform-properties',
    summary: 'Enroll or update a social or video Search Console property',
    description: 'Best-effort bridge for Google\'s gradual platform-property rollout. Creation and verification stay in Search Console. Canonry accepts only an exact verified sites.list identifier for the connected account and rejects the project\'s primary website property.',
    tags: ['google'], parameters: [nameParameter],
    requestBody: { required: true, content: { 'application/json': { schema: { $ref: '#/components/schemas/GscPlatformPropertyUpsertRequestDto' } } } },
    responses: { 200: jsonResponse('Platform property saved.', 'GscPlatformPropertyDto'), 400: errorResponse('Invalid or unavailable Search Console property.') },
  },
  {
    method: 'delete',
    path: '/api/v1/projects/{name}/google/gsc/platform-properties/{id}',
    summary: 'Remove an enrolled social or video Search Console property',
    tags: ['google'], parameters: [nameParameter, { name: 'id', in: 'path', required: true, description: 'Platform property ID.', schema: stringSchema }],
    responses: { 204: { description: 'Platform property deleted.' }, 404: errorResponse('Platform property not found.') },
  },
  {
    method: 'post',
    path: '/api/v1/projects/{name}/google/gsc/platform-properties/{id}/sync',
    summary: 'Queue a sync for one social or video Search Console property',
    description: 'Queries the standard Search Analytics API only. Website GSC data, URL inspection, coverage, and sitemaps are untouched. An existing queued/running sync for this property is returned instead of starting another.',
    tags: ['google'], parameters: [nameParameter, { name: 'id', in: 'path', required: true, description: 'Platform property ID.', schema: stringSchema }],
    responses: { 200: jsonResponse('Platform sync run returned.', 'RunDto'), 404: errorResponse('Platform property not found.') },
  },
  {
    method: 'get',
    path: '/api/v1/projects/{name}/google/gsc/platform-performance',
    summary: 'Get social and video Search Console performance',
    description: 'Returns property-level totals and daily trend separately from paginated page/query detail rows. Supports all bound properties or one Canonry property ID.',
    tags: ['google'],
    parameters: [
      nameParameter,
      { name: 'propertyId', in: 'query', description: 'Optional enrolled property ID.', schema: stringSchema },
      { name: 'dimension', in: 'query', description: 'Aggregate by page or query.', schema: { type: 'string', enum: ['page', 'query'] } },
      { name: 'startDate', in: 'query', description: 'Filter by start date.', schema: { type: 'string', format: 'date' } },
      { name: 'endDate', in: 'query', description: 'Filter by end date.', schema: { type: 'string', format: 'date' } },
      analyticsWindowParameter,
      {
        name: 'limit',
        in: 'query',
        description: 'Maximum number of dimension rows to return.',
        schema: { type: 'integer', minimum: 1, maximum: 500 },
      },
      offsetQueryParameter,
    ],
    responses: { 200: jsonResponse('Platform performance returned.', 'GscPlatformPerformanceDto'), 400: errorResponse('Invalid performance query.'), 404: errorResponse('Project or platform property not found.') },
  },
  {
    method: 'get',
