...
import { buildMentionExtractPrompt, parseMentionExtractResponse } from '../src/visibility/mention-extract.js'
import { namesWrittenIn } from '../src/visibility/brand.js'
import { computeMentionShare, computeShareOfVoice, SHARE_MAX_ROWS } from '../src/visibility/share.js'
import { checkFingerprint } from '../src/runtime/records.js'
import type { VisibilityEvidence } from '../src/runtime/types.js'
...
test('a brand the answer does not write is dropped, whatever the model returned', () => {
...
test('matching is by whole words, never by substring', () => {
...
test('the target is counted from its own verdict, not from the extraction', () => {
...
test('one rival named in different casing across answers is a single row', () => {
...
test('the target is not double-counted when the prose cases its name differently', () => {
...
test('an answer the extraction never covered is excluded, not counted as naming nobody', () => {
...
test('nothing measurable returns null rather than an empty table', () => {
...
test('the two bases are independent tables and never blend', () => {
...
test('both bases share one ranker, so ordering and the cut cannot drift', () => {
...
test('a malformed extraction response is unmeasured, never an empty finding', () => {
...
test('the extraction parser holds each answer to its own index', () => {
...
test('the prompt forbids the inference the metric is built to avoid', () => {
...
test('adding a measured signal retires the records that lack it', () => {
