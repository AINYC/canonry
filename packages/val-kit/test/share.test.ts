...
import { computeShareOfVoice, SHARE_MAX_ROWS } from '../src/visibility/share.js'
...
test('share is the fraction of answer appearances, and the parts sum to one', () => {
...
test('a domain cited many times in one answer still counts once', () => {
...
test('the checked site absorbs its own subdomains', () => {
  const share = computeShareOfVoice([
...
test('a www.-prefixed target still groups its own citations', () => {
...
test('a site that is never cited is a measured zero, not a missing row', () => {
...
test('the tail is a stated fact, never a row with a nonsense ratio', () => {
...
test('a failed answer is unmeasured and cannot dilute the share', () => {
...
test('a successful answer with no attributable source is counted, not hidden', () => {
...
test('nothing measurable returns null rather than an empty chart', () => {
...
test('the checked site keeps its row even when it ranks below the cut', () => {
  const answers = [
...
test('ordering is deterministic so the same answers always draw the same bar', () => {
