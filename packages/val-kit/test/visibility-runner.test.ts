...
import type { VisibilityProviderAdapter } from '../src/visibility/contracts.js'
...
test('keeps answer mentions and source citations as independent evidence signals', async () => {
...
test('keeps failed checks null and excludes them from mention and citation denominators', async () => {
  const report = await runVisibilityProbe({
    target,
    queries: [{ id: 'q1', text: 'best answer engine optimization tools' }],
    adapters: [
...
test('treats blank provider answers as invalid observations instead of negative results', async () => {
  const report = await runVisibilityProbe({
    target,
    queries: [{ id: 'q1', text: 'best answer engine optimization tools' }],
...
test('turns pre-aborted provider work into a failed null observation', async () => {
...
test('bounds public query, provider, and model identities', async () => {
  await rejects(
    () =>
      runVisibilityProbe({
        target,
...
test('clips every public citation URL and retains a target match before source caps', async () => {
...
test('does not match a domain label as prose or an alias as a substring', async () => {
  const report = await runVisibilityProbe({
...
test('counts an explicit written canonical domain as an answer mention', async () => {
  const report = await runVisibilityProbe({
