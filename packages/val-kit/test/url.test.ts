...
import { normalizePublicDomain, PublicUrlError, wwwAlternate } from '../src/security/url.js'
...
test('normalizes a public bare domain to an HTTPS root', () => {
...
test('rejects paths, credentials, private ranges, and local hosts', () => {
  for (
    const input of [
...
test('rejects special-use internal name suffixes, including cloud metadata', () => {
  for (
    const host of [
...
test('only applies IPv6 private prefixes to IPv6 literals and rejects public literals deliberately', () => {
...
test('blocks only the 192.0.0.0 IPv4 /24 special range', () => {
...
test('only derives a domain-www alternate for DNS names', () => {
