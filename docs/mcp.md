...
The full 151-tool catalog (149 API tools plus two meta-tools) is too large to expose eagerly in most sessions. `canonry-mcp` defaults to a small **core tier** and registers the rest on demand via `notifications/tools/list_changed`.
...
| `gsc` | Google connections, website performance, inspections, coverage, sitemaps, plus social/video property list, bind, remove, sync, and performance | Search performance and indexing work; platform-property API compatibility is best-effort until Google documents or exposes an account's rollout |
...
Power-user environments (scripts, Aero, telemetry harnesses) that want the flat 151-tool catalog at startup can opt back in with `--eager` (or `CANONRY_MCP_EAGER=1`):
