...
The full 189-tool catalog (187 API tools plus two meta-tools) is too large to expose eagerly in most sessions. `canonry-mcp` defaults to a small **core tier** and registers the rest on demand via `notifications/tools/list_changed`.
...
Power-user environments (scripts, Aero, telemetry harnesses) that want the flat 179-tool catalog at startup can opt back in with `--eager` (or `CANONRY_MCP_EAGER=1`):
