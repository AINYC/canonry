...
The full 154-tool catalog (152 API tools plus two meta-tools) is too large to expose eagerly in most sessions. `canonry-mcp` defaults to a small **core tier** and registers the rest on demand via `notifications/tools/list_changed`.
...
| `setup` | project export/upsert, measurement-plan get/history/compile/diff/publish, queries list/add/remove/replace/generate, legacy keyword aliases, competitors list/add/remove, schedule get/set/delete, insight dismiss, backlinks domains | Onboarding a project, reviewing and publishing its Target measurement plan, editing queries/competitors/schedules, reviewing backlink coverage |
...
Power-user environments (scripts, Aero, telemetry harnesses) that want the flat 154-tool catalog at startup can opt back in with `--eager` (or `CANONRY_MCP_EAGER=1`):
