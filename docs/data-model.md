...
| **oauth_clients** | Pre-registered OAuth 2.1 clients for the remote MCP surface. DCR (RFC 7591) is deliberately not implemented, so an operator registers these explicitly and nothing on the network can mint one. `secretHash` is NULL for a public client authenticating by PKCE alone. `redirectUris` is an exact-match allowlist |
