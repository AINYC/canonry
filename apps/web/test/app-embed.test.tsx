...
test('embed defaults client-value overview disclosures open and omits run history', async () => {
  ...
  expect(detailsForTitle(operatorDoc, 'Query changes')?.hasAttribute('open')).toBe(false)
  ...
  expect(detailsForTitle(embedDoc, 'Query changes')?.hasAttribute('open')).toBe(true)
