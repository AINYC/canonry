...
async function renderAt(
  ...
  if (measurement?.competitorLandscape) {
    const q = {
      ...
      ...(measurement.plan.active?.plan.schemaVersion === 2
        ? { queryClass: measurement.competitorLandscapeKey?.queryClass ?? 'all' }
        : {}),
...
test('saving the project provider allowlist refreshes server-owned sweep readiness', async () => {
  ...
  fireEvent.click(await page.findByLabelText('All configured native engines'))
