...
export function EngineRoutesSettings() {
  ...
  return (
    <section aria-labelledby="engine-routes-title" className="page-section-divider">
      ...
      <p className="mt-2 text-sm text-secondary">Text-only routes can support research. Native answer engines run visibility sweeps.</p>
...
function RouteEditor({ route, existingRouteIds, connections, onCancel, onSaved }: {
  ...
  return (
    <form className="mt-5 border-y border-default py-5" onSubmit={asyncHandler(async event => { event.preventDefault(); await save() })} aria-label={route ? `Edit ${route.label}` : 'Add route'}>
      ...
      <p className="text-sm text-secondary">This route supports research. It cannot run answer-visibility sweeps.</p>
