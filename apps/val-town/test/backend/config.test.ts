...
Deno.test('production fails closed when quota salt is absent', () => {
...
Deno.test('production requires every Turnstile hostname/site-key/secret component', () => {
  const config = loadValTownConfig({
    VAL_TOWN_ENV: 'production',
...
Deno.test('explicit development bypass does not become a production default', () => {
